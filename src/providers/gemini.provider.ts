...
export class GeminiProvider implements ILLMProvider {
  ...
  constructor() {
    this.apiKeys = parseApiKeys(env.GEMINI_API_KEYS || '');
    if (this.apiKeys.length === 0) {
      log.warn('Gemini provider initialized with no API keys.');
    }
  }

  private checkConfigured() {
    if (this.apiKeys.length === 0) {
      throw new Error('Gemini provider not configured — add GEMINI_API_KEYS to .env');
    }
  }
  ...
  async *streamText(messages: LLMMessage[], options?: StreamOptions): AsyncGenerator<StreamChunk> {
    ...
    throw new Error('GeminiProvider.streamText not implemented');
  }
  ...
  async generateEmbeddings(text: string | string[]): Promise<EmbeddingResult[]> {
    this.checkConfigured();
    throw new Error('GeminiProvider.generateEmbeddings not implemented');
  }
}
