...
import { FileTree, FileNode, INITIAL_PROJECT_TREE } from "./file-tree";
...
export default function TopNavMenu({ 
  ...
  const [activeTab, setActiveTab] = useState<"search" | "tree">("tree");
  ...
  const content = useMemo(() => {
    switch (view) {
      ...
      case "folders":
        return (
          <div className="space-y-2.5 min-w-[310px] max-w-[340px] p-2.5">
            {/* View Switcher: File Tree Explorer vs Search Sidebar */}
            ...
            {activeTab === "tree" ? (
              /* Interactive Recursive File Tree */
              <div className="pt-1">
                <FileTree
                  activeFileId={activeViewerFile?.id}
                  onSelectFile={handleFileSelect}
                  onAddFolder={onAddFolder}
                  theme={theme}
                  className="max-w-none shadow-none border-0 bg-transparent p-0"
                />
              </div>
            ) : (
              /* Search & Sort Panel */
  ...
  }, [view, theme, isLight, isInitialGreeting, activeTab, activeViewerFile, onNewChat, onAddFolder, onArgueModel, onThemeChange, sharedHover, isFilterSubmenuOpen, searchFolderQuery]);
  ...
  return (
    <>
      ...
      {/* Multi-Format File Viewer Modal */}
      <FileViewerModal
